...
1. **Minimal Dependencies** - Only Bash + Node.js required (Node is used solely for search index generation)
2. **Runs Everywhere** - Works on any system with Bash + Node.js (Linux, Mac, WSL, GitHub Actions)
3. **Easy to Understand** - ~70 lines of readable shell script
...
│   ├── apim-internal-endpoints.html # APIM internal endpoints guide
...
│   ├── services.html          # Tenant-facing services and model catalog
...
│   └── search.js              # Client-side search UI (modal, breadcrumbs, FlexSearch wiring)
│
├── plans/                     # Working notes for upcoming docs changes
├── build.sh                   # Template engine + search index build script
├── generate-search-index.js   # Node.js script: parses built HTML → writes search-index.json
├── generate-tf-docs.sh        # Terraform docs generator
├── README.md                  # This file
│
└── [generated HTML files]     # Output (published to GitHub Pages)
    ├── apim-internal-endpoints.html
...
### Step 4: Concatenation + Search Index

The final page is assembled, then `build.sh` calls `generate-search-index.js` via Node.js:
...
node generate-search-index.js . # Writes assets/search-index.json
```

`generate-search-index.js` scans every built HTML page, auto-assigns `id=` attributes to any heading missing one, and writes `assets/search-index.json`. The browser loads this file on the first search and feeds it into [FlexSearch](https://github.com/nextapps-de/flexsearch) (loaded from CDN) for fast full-text querying.
...
> **Requires:** Bash and Node.js (v24+)

```bash
cd docs

# Build all pages + search index
./build.sh
    ...
    steps:
      ...
      - name: Build documentation
        run: |
          ...
          ./build.sh             # Builds HTML + search-index.json
