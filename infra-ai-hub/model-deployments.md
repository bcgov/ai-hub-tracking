...
Mistral models are serverless MaaS (pay-per-token). The Foundry deployment UI reports a 10M TPM total quota for `Mistral-Large-3`; the document AI model does not yet have an agreed shared allocation target in this repo.
...
The following 5 models were tested and excluded:
...
- `mistral-document-ai-2505` — **retired**; Azure ended inference on 2026-07-20 and the model is `Deprecated` in Canada East. Superseded by `mistral-document-ai-2512`. The deployment stayed `Succeeded` in ARM after retirement, so `terraform apply` kept passing while OCR calls failed at the data plane.

2 Mistral models are serverless MaaS (pay-per-token). Based on the Foundry deployment settings, `Mistral-Large-3` has a 10M TPM total quota and `ai-hub-admin` now reserves 1% of that total. The document AI model remains at the minimal allocation until a separate quota target is defined.
