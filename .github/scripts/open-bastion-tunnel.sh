...
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] open-bastion-tunnel.sh started"
...
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Resolving Bastion resource group and subscription..."
...
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Discovering Bastion host..."
BASTION_NAME="${BASTION_NAME:-$(az network bastion list -g "$RG" --subscription "$SUB" --query '[0].name' -o tsv)}"
[[ -n "$BASTION_NAME" ]] || { echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] ERROR: No Bastion found in $RG — run ensure-bastion.sh first." >&2; exit 1; }
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Bastion host: $BASTION_NAME"
VM_ID="${VM_ID:-$(az vm list -g "$RG" --subscription "$SUB" --query '[0].id' -o tsv)}"
[[ -n "$VM_ID" ]] || { echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] ERROR: No jumpbox VM found in $RG — cannot open the tunnel." >&2; exit 1; }

echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Jumpbox VM: $VM_ID"

echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Installing Azure CLI extensions (bastion, ssh)..."
az extension add --name bastion --yes
az extension add --name ssh --yes
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Extensions installed."
...
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Opening SOCKS5 tunnel on port $SOCKS..."
...
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Waiting for SOCKS tunnel to be ready (up to 60 sec)..."
for i in $(seq 1 30); do
  nc -z 127.0.0.1 "$SOCKS" && { echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] SOCKS tunnel established (attempt $i/30)."; break; }
  echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] SOCKS tunnel not ready yet ($i/30), waiting..."
  sleep 2
done
nc -z 127.0.0.1 "$SOCKS" || {
  echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] ERROR: SOCKS tunnel failed to open on port $SOCKS:"
  cat "$TUNNEL_LOG" || true
  exit 1
}
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] SOCKS5 tunnel open on localhost:$SOCKS"
...
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] Starting privoxy container..."
docker run -d --name privoxy --network host \
  -e SOCKS_HOST=127.0.0.1 -e SOCKS_PORT="$SOCKS" \
  "$PRIVOXY_IMAGE"
docker logs privoxy
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] privoxy HTTP bridge ready on localhost:8118"
echo "[$(date -u +'%Y-%m-%d %H:%M:%S')] open-bastion-tunnel.sh completed successfully."
